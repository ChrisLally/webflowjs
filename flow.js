...
#world {
	...
	height: 100%;
	overflow: hidden;
	background: linear-gradient(#D31EFF, #f7d9aa);
}
</style>
...
function createScene() {
	...
	scene.fog = new THREE.Fog(0xFFEE38, 100, 950);
...
var y = canvas.height / 2+61;
context.font = "123pt Calibri bold";
context.textAlign = "center";
context.fillStyle = "#FF2B79";
...
xm.map.needsUpdate = true;
  ...
  var geomTailFlag = new THREE.BoxGeometry(250,120,5,1,1,1);
  var matTailFlag = new THREE.MeshPhongMaterial({color:Colors.red, shading:THREE.FlatShading});
  var tailFlag = new THREE.Mesh(geomTailFlag,xm);
  tailFlag.position.set(-220,20,0);
  tailFlag.castShadow = true;
  tailFlag.receiveShadow = true;
  this.mesh.add(tailFlag);
...
function updatePlane() {
	...
	airplane.mesh.position.x += (targetX-airplane.mesh.position.x)*0.1+5;
